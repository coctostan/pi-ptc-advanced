...
test("package.json targets pi-ptc-advanced@0.18.0", () => {
  const pkg = JSON.parse(read("package.json")) as { name?: string; version?: string };
  assert.equal(pkg.name, EXPECTED_PACKAGE_NAME);
  assert.equal(pkg.version, EXPECTED_VERSION);
});

test("package-lock.json root metadata is aligned to 0.18.0", () => {
...
test("scripts/verify-release-package.sh asserts the 0.18.0 baseline", () => {
  const script = read("scripts/verify-release-package.sh");
  assert.ok(
    script.includes(`pkg.version !== '${EXPECTED_VERSION}'`),
    "verify-release-package.sh must check version 0.18.0",
  );
  assert.ok(
    script.includes(`expected version ${EXPECTED_VERSION}`),
    "verify-release-package.sh must reference version 0.18.0 in its error text",
  );
  assert.ok(
...
test("docs/releases/0.18.0.md exists and describes Milestone 19 outcomes", () => {
  assert.ok(exists(EXPECTED_RELEASE_DOC), `${EXPECTED_RELEASE_DOC} must exist`);
  const note = read(EXPECTED_RELEASE_DOC);
  assert.ok(/0\.18\.0/.test(note), "release note must mention 0.18.0");
  assert.ok(
...
test("README references the 0.18.0 release baseline and release note", () => {
  const readme = read("README.md");
  assert.ok(
    readme.includes(`${EXPECTED_PACKAGE_NAME}@${EXPECTED_VERSION}`),
    "README must reference pi-ptc-advanced@0.18.0",
  );
  assert.ok(
    readme.includes(`docs/releases/${EXPECTED_VERSION}.md`),
    "README must link to the 0.18.0 release note",
  );
  assert.ok(
...
test("CHANGELOG promotes the 0.18.0 release and records the audit caveat", () => {
  const changelog = read("CHANGELOG.md");
  assert.ok(
    /^## 0\.18\.0/m.test(changelog),
    "CHANGELOG must contain a 0.18.0 release section",
  );
  const section = changelog.split(/^## 0\.18\.0/m)[1] ?? "";
  assert.ok(/audit/i.test(section), "0.18.0 section must mention the audit caveat");
  assert.ok(
    /(report|helper|run_tests|code_execution|ptc\.report)/i.test(section),
    "0.18.0 section must reference report/helper work",
  );
});

test("docs/personal-fork-maintenance.md targets the 0.18.0 baseline", () => {
  const runbook = read("docs/personal-fork-maintenance.md");
  assert.ok(
    runbook.includes(`${EXPECTED_PACKAGE_NAME}@${EXPECTED_VERSION}`),
    "runbook must reference pi-ptc-advanced@0.18.0",
  );
  assert.ok(
    runbook.includes(`releases/${EXPECTED_VERSION}.md`),
    "runbook must link to the 0.18.0 release note",
  );
  assert.ok(
